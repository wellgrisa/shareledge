...
function handleListGroup(){
	...
	listGroup.delegate(".answer-collapsible", "show.bs.collapse", function(){
		...
		if(selectedQuestion.hasClass('list-group-item-highlighted')){
...
function updatePagination(paginationOfQuestions){
	var pagination = $('.pagination');

	pagination.html('');

	for (var i = 1; i <= paginationOfQuestions.totalPages; i++) {

		if(paginationOfQuestions.currentPage == i)
...
function getQuestionsByLink(done){

	var linkId = 'all-questions';

	var pageNumber =  2;

	getOutstandingCountByFilter({filter : { criteria :{type :$('#systems').val()}, page : pageNumber }}, refreshQuestionsWith, done);
}

function getOutstandingCountByFilter(filter, callback, done){
	$.getJSON( '/questions/outstandingFilter', filter)
	.done(function(json){
		if(done){
			done({ data : json.records});
		}
	})
